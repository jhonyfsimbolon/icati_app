...
import 'package:icati_app/module/login/login_otp_wa/pattern/LoginOtpWaPresenter.dart';
import 'package:icati_app/module/login/login_otp_wa/pattern/LoginOtpWaView.dart';
import 'package:icati_app/module/login/login_otp_wa/pattern/LoginOtpWaWidgets.dart';
import 'package:icati_app/module/menubar/MenuBar.dart';

class LoginOtpWa extends StatefulWidget {
  String type;

  LoginOtpWa(this.type);

  @override
  _LoginOtpWaState createState() => _LoginOtpWaState();
}

class _LoginOtpWaState extends State<LoginOtpWa> implements LoginOtpWaView {
  LoginOtpWaPresenter _loginOtpWaPresenter;
  static final _formKey = new GlobalKey<FormState>();

  TextEditingController _waCont = TextEditingController();
  ...
  @override
  void initState() {
    _loginOtpWaPresenter = LoginOtpWaPresenter();
    _loginOtpWaPresenter.attachView(this);
    super.initState();
    _getCredential();
  }
                    ...
                    widget.type == "sendOtpWa"
                        ? Column(
                            children: [
                              ...
                              SizedBox(height: 10),
                              Text("MASUK DENGAN OTP WHATSAPP",
                          ...
                          Form(
                            ...
                            child: Column(
                              children: <Widget>[
                                widget.type == "sendOtpWa"
                                    ? LoginOtpWaWidgets()
                                        .displayLoginOtpWa(context, _waCont)
                                    : LoginOtpWaWidgets().displayWaOtpInput(
  ...
  _sendOtpWa() async {
    ...
    if (_formKey.currentState.validate()) {
      _loginOtpWaPresenter.sendLoginOtpWa(_waCont.text);
    } else {
      setState(() {
              ...
              if (_baseData.resendOTP == true) {
                setState(() {
                  ...
                  if (_dataOtpLogin != null) {
                    _loginOtpWaPresenter.sendLoginOtpWa(_dataOtpLogin[0]['wa']);
                  }
                });
              }
            },
  ...
  @override
  onSuccessSendOtpWa(Map data) async {
    if (this.mounted) {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      setState(() {
        _baseData.isSaving = false;
        ...
        if (widget.type == "verifyOtpWa") {
          startCount();
        }

        final _dataOtpWa = prefs.getString("otpWa");
        final Map _dataOtp = jsonDecode(_dataOtpWa);
        _dataOtpLogin = _dataOtp['otpwa'];
        print("ini data otp " + _dataOtp['otpwa'].toString());
      });
    }
  }

  @override
  onFailLoginOtpWa(Map data) {
    if (this.mounted) {
  ...
  @override
  onSuccessLoginOtpWa(Map data) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (this.mounted) {
      setState(() {
